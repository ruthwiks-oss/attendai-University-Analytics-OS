...
export async function itemDelete(model: Model<unknown>, id: string) {
  try { return await deleteDocument(model, id); } catch { return NextResponse.json({ success: false, error: "Request could not be completed." }, { status: 400 }); }
}
